...
void Mundo::inicializa(int estado) {
    ...
    tablero.vaciar(); // Asume que tienes un m�todo para poner todas las casillas a nullptr
    ...
    switch (estado) {
    case 0: // INICIO: No creamos nada, el tablero se queda vac�o y limpio
        break;
    case 2: // MENU: 
        // ...
        break;
...
void Mundo::dibuja(int estado) {
    ...
    glLoadIdentity();
    glOrtho(-10, 10, -10, 10, -1, 1); // Versi�n m�s robusta de Ortho
   ...
   switch (estado) {
       // 1. CONFIGURACI�N DE C�MARA (FUERA DEL SWITCH)
       glMatrixMode(GL_PROJECTION);
       glLoadIdentity();
    ...
    case 1: // ESTADO MENU
        // Dibujamos el tablero normal pero quiz�s sin el rat�n rojo
        tablero.dibuja(1.0f);
        break;

    case 2: // ESTADO JUEGO (El que ya ten�as hecho)
        ...
        raton.dibuja(); // Aqu� s� queremos ver el rat�n
        break;
    ...
    case 3: // ESTADO PAUSA
        ...
        if (seleccionada != nullptr) {
            // Podemos a�adir un m�todo a Raton para cambiar color o hacerlo aqu�
