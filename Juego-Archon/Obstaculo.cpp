...
bool Obstaculo::colisionaCon(Pieza* p) const {
    if (p == nullptr) return false;

    // Calculamos la distancia entre el obstáculo y la pieza
    Vector2D dist = posicion - p->obtenerPosicion();

    // Si la distancia es menor que la suma de sus "radios", se están tocando.
    // Asumimos un radio de colisión aproximado para la pieza (ej: 1.0f)
